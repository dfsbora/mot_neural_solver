...
def main(
    ...
    obj_detect.load_state_dict(
        torch.load(
            osp.join(OUTPUT_PATH, frcnn_weights),
    ...
    for dataset_name in dataset_names:
        ...
        for seq in dataset:
            ...
            if prepr_w_tracktor:
                preprocessor.do_align = tracktor_params["tracker"]["do_align"] and (
                    MOV_CAMERA_DICT[str(seq)]
                )

            data_loader = DataLoader(
                seq, batch_size=1, shuffle=False, num_workers=6, pin_memory=True
            )
            ...
            output_file_path = osp.join(
                seq.seq_path, "det", prepr_params["det_file_name"]
            )
            if prepr_w_tracktor:
