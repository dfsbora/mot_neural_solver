...
class MOTGraphAugmentor:
    ...
    def _drop_ids(self):
        ...
        ids_to_drop_perc = np.random.uniform(
            self.dataset_params["min_ids_to_drop_perc"],
        ...
        self.graph_df = (
            self.graph_df[~self.graph_df.id.isin(ids_to_drop)]
    ...
    def _drop_detections(self):
        ...
        detects_to_drop_perc = np.random.uniform(
            self.dataset_params["min_detects_to_drop_perc"],
            self.dataset_params["max_detects_to_drop_perc"],
        )
        num_detects_to_drop = np.round(detects_to_drop_perc * len(all_detects)).astype(
            int
        )

        # Randomly choose which detections to drop and update self.graph_df
        detects_to_drop = np.random.choice(
            all_detects, num_detects_to_drop, replace=False
        )
    ...
    def _wiggle_boxes(self):
        ...
        col_shift_dict = {
            "bb_top": bb_top_shift,
            "bb_bot": bb_bot_shift,
            "bb_left": bb_left_shift,
            "bb_right": bb_right_shift,
        }
        ...
        iou_val = iou_pairs(
            self.graph_df[iou_cols].values.T, original_graph_df[iou_cols].values.T
        )
