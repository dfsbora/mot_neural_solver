...
from mot_neural_solver.data.seq_processing.MOTCha_loader import (
    get_mot_det_df,
    get_mot_det_df_from_gt,
)
from mot_neural_solver.data.seq_processing.MOT15_loader import (
    get_mot15_det_df,
    get_mot15_det_df_from_gt,
)
...
_ENSURE_BOX_IN_FRAME = {"MOT": False, "MOT_gt": False, "MOT15": True, "MOT15_gt": False}
...
# MOT20 Sequences
mot20_seqs = [
    f"MOT20-{seq_num:02}{det}" for seq_num in (1, 2, 3, 5) for det in ("", "-GT")
]
...
# MOT17 Sequences
mot17_seqs = [
    f"MOT17-{seq_num:02}-{det}"
    for seq_num in (2, 4, 5, 9, 10, 11, 13)
    for det in ("DPM", "SDP", "FRCNN", "GT")
]
mot17_seqs += [
    f"MOT17-{seq_num:02}-{det}"
    for seq_num in (1, 3, 6, 7, 8, 12, 14)
    for det in ("DPM", "SDP", "FRCNN")
]
...
class MOTSeqProcessor:
    ...
    def __init__(
        self, dataset_path, seq_name, dataset_params, cnn_model=None, logger=None
    ):
    ...
    def _assign_gt(self):
        ...
        """
        if (
            self.det_df.seq_info_dict["has_gt"]
            and not self.det_df.seq_info_dict["is_gt"]
        ):
            print(
                f"Assigning ground truth identities to detections to sequence {self.seq_name}"
            )
            for frame in self.det_df["frame"].unique():
                ...
                iou_matrix = iou(
                    frame_detects[["bb_top", "bb_left", "bb_bot", "bb_right"]].values,
                    frame_gt[["bb_top", "bb_left", "bb_bot", "bb_right"]].values,
                )

                iou_matrix[
                    iou_matrix < self.dataset_params["gt_assign_min_iou"]
                ] = np.nan
                dist_matrix = 1 - iou_matrix
                assigned_detect_ixs, assigned_detect_ixs_ped_ids = solve_dense(
                    dist_matrix
                )
                unassigned_detect_ixs = np.array(
                    list(set(range(frame_detects.shape[0])) - set(assigned_detect_ixs))
                )

                assigned_detect_ixs_index = frame_detects.iloc[
                    assigned_detect_ixs
                ].index
                assigned_detect_ixs_ped_ids = frame_gt.iloc[
                    assigned_detect_ixs_ped_ids
                ]["id"].values
                unassigned_detect_ixs_index = frame_detects.iloc[
                    unassigned_detect_ixs
                ].index

                self.det_df.loc[
                    assigned_detect_ixs_index, "id"
                ] = assigned_detect_ixs_ped_ids
                self.det_df.loc[
                    unassigned_detect_ixs_index, "id"
                ] = -1  # False Positives
    ...
    def _get_det_df(self):
        ...
        self.det_df, seq_info_dict, self.gt_df = self.det_df_loader(
            self.seq_name, self.dataset_path, self.dataset_params
        )
        ...
        if (
            self.seq_type in _ENSURE_BOX_IN_FRAME
            and _ENSURE_BOX_IN_FRAME[self.seq_type]
        ):
    ...
    def _store_df(self):
        ...
        """
        processed_dets_path = osp.join(
            self.det_df.seq_info_dict["seq_path"], "processed_data", "det"
        )
        os.makedirs(processed_dets_path, exist_ok=True)
        det_df_path = osp.join(
            processed_dets_path, self.det_df.seq_info_dict["det_file_name"] + ".pkl"
        )
        self.det_df.to_pickle(det_df_path)
        print(
            f"Finished processing detections for seq {self.seq_name}. Result was stored at {det_df_path}"
        )
    ...
    def _store_embeddings(self):
        ...
        assert (
            self.dataset_params["reid_embeddings_dir"] is not None
            and self.dataset_params["node_embeddings_dir"] is not None
        )

        # Create dirs to store embeddings
        node_embeds_path = osp.join(
            self.det_df.seq_info_dict["seq_path"],
            "processed_data/embeddings",
            self.det_df.seq_info_dict["det_file_name"],
            self.dataset_params["node_embeddings_dir"],
        )

        reid_embeds_path = osp.join(
            self.det_df.seq_info_dict["seq_path"],
            "processed_data/embeddings",
            self.det_df.seq_info_dict["det_file_name"],
        ...
        max_dets_per_df = int(
            1e5
        )  # Needs to be larger than the maximum amount of dets possible to have in one frame

        frame_cutpoints = [
            self.det_df.frame.iloc[i]
            for i in np.arange(0, num_dets, max_dets_per_df, dtype=int)
        ]
        ...
        for frame_start, frame_end in zip(frame_cutpoints[:-1], frame_cutpoints[1:]):
            ...
            bbox_dataset = BoundingBoxDataset(
                sub_df,
                seq_info_dict=self.det_df.seq_info_dict,
                return_det_ids_and_frame=True,
            )
            bbox_loader = DataLoader(
                bbox_dataset,
                batch_size=self.dataset_params["img_batch_size"],
                pin_memory=True,
                num_workers=4,
            )
    ...
    def load_or_process_detections(self):
        ...
        det_file_to_use = (
            self.dataset_params["det_file_name"]
            if not self.seq_name.endswith("GT")
            else "gt"
        )
        seq_det_df_path = osp.join(
            seq_path, "processed_data/det", det_file_to_use + ".pkl"
        )
        ...
        embeds_ok = (
            osp.exists(node_embeds_path)
            and len(os.listdir(node_embeds_path)) == num_frames
        )
        embeds_ok = (
            embeds_ok
            and osp.exists(reid_embeds_path)
            and len(os.listdir(reid_embeds_path)) == num_frames
        )
        embeds_ok = embeds_ok or not self.dataset_params["precomputed_embeddings"]

        if (
            processed_dets_exist
            and embeds_ok
            and not self.dataset_params["overwrite_processed_data"]
        ):
            print(
                f"Loading processed dets for sequence {self.seq_name} from {seq_det_df_path}"
            )
            seq_det_df = (
                pd.read_pickle(seq_det_df_path)
                .reset_index()
                .sort_values(by=["frame", "detection_id"])
            )

        else:
            print(
                f"Detections for sequence {self.seq_name} need to be processed. Starting processing"
            )
