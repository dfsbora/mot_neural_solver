...
MOV_CAMERA_DICT.update(
    {f"MOT20-{seq_num:02}{gt}": False for seq_num in range(1, 9) for gt in ("", "-GT")}
)
...
def _add_frame_path_mot17(det_df, seq_name, data_root_path):
    ...
    add_frame_path = lambda frame_num: osp.join(
        det_seq_path, det_seq_path, f"img1/{frame_num:06}.jpg"
    )
...
def get_mot_det_df(seq_name, data_root_path, dataset_params):
    ...
    seq_path = osp.join(data_root_path, seq_name)
    detections_file_path = osp.join(
        seq_path, f"det/{dataset_params['det_file_name']}.txt"
    )
    ...
    if "MOT17" in seq_name:
        ...
        det_df["frame_path"] = det_df["frame"].apply(
            lambda frame_num: osp.join(seq_path, f"img1/{frame_num:06}.jpg")
        )

    assert osp.exists(det_df["frame_path"].iloc[0])

    seq_info_dict = _build_scene_info_dict_mot17(
        seq_name, data_root_path, dataset_params
    )
    seq_info_dict["is_gt"] = False
    if seq_info_dict[
        "has_gt"
    ]:  # Return the corresponding ground truth, if available, for the ground truth assignment
        ...
        gt_df["bb_top"] -= 1
        gt_df = gt_df[
            gt_df["label"].isin([1, 2, 7, 8, 12])
        ].copy()  # Classes 7, 8, 12 are 'ambiguous' and tracking
...
def get_mot_det_df_from_gt(seq_name, data_root_path, dataset_params):
    ...
    if not osp.exists(seq_path):
        ...
        if (
            "MOT17" in seq_name
        ):  # For MOT17 we use e.g. the seq with DPM detections (any will do)
        ...
        shutil.copy(
            osp.join(src_seq_path, "seqinfo.ini"), osp.join(seq_path, "seqinfo.ini")
        )
