...
def get_mot15_det_df(seq_name, data_root_path, dataset_params):
    ...
    add_frame_path = lambda frame_num: osp.join(
        data_root_path, seq_name, f"img1/{frame_num:06}.jpg"
    )
...
def get_mot15_det_df_from_gt(seq_name, data_root_path, dataset_params):
    ...
    add_frame_path = lambda frame_num: osp.join(
        data_root_path, seq_name[:-3], f"img1/{frame_num:06}.jpg"
    )
    det_df["frame_path"] = det_df["frame"].apply(add_frame_path)

    seq_info_dict = _build_seq_info_dict_mot15(
        seq_name[:-3], data_root_path, dataset_params
    )
...
def drop_occluded_gt_annotations(gt_df, dataset_params):
    ...
    time_dist_matrix = gt_df.frame.values.reshape(-1, 1) - gt_df.frame.values.reshape(
        1, -1
    )
    not_same_frame_ixs = np.where(time_dist_matrix != 0)
    same_detect_ixs = np.diag_indices(gt_df.shape[0])
    invalid_ixs = tuple(
        [np.concatenate((not_same_frame_ixs[i], same_detect_ixs[i])) for i in [0, 1]]
    )
    ...
    occluded_matrix = intersec_over_min_max_area(
        gt_df[["bb_top", "bb_left", "bb_bot", "bb_right"]].values,
        gt_df[["bb_top", "bb_left", "bb_bot", "bb_right"]].values,
        denom_operator="min",
    )
    ...
    smaller_area = gt_df.bb_size.values.reshape(1, -1) < gt_df.bb_size.values.reshape(
        -1, 1
    )  # True if
    # row < column??
    ...
    drop_step_2 = (
        (occluded_matrix > dataset_params["GT_train_max_iou_containment_thresh"])
        & smaller_area
    ).max(axis=0)
