...
from mot_neural_solver.utils.rgb import (
    load_embeddings_from_imgs,
...
class MOTGraph(object):
    ...
    def _load_appearance_data(self):
        ...
        if self.inference_mode and not self.dataset_params["precomputed_embeddings"]:
            ...
            reid_embeds = load_precomputed_embeddings(
                det_df=self.graph_df,
                seq_info_dict=self.seq_info_dict,
            ...
            if (
                ...
                node_feats = load_precomputed_embeddings(
                    det_df=self.graph_df,
                    seq_info_dict=self.seq_info_dict,
    ...
    def assign_edge_labels(self):
        ...
        ids = torch.as_tensor(
            self.graph_df.id.values, device=self.graph_obj.edge_index.device
        )
        per_edge_ids = torch.stack(
            [ids[self.graph_obj.edge_index[0]], ids[self.graph_obj.edge_index[1]]]
        )
        ...
        active_fut_edges = scatter_min(
            time_dists[future_mask],
            same_id_edges[0][future_mask],
            dim=0,
            dim_size=self.graph_obj.num_nodes,
        )[1]
        ...
        active_fut_edges = original_node_ixs[
            active_fut_edges
        ]  # Recover the node id of the corresponding
        ...
        active_past_edges = scatter_min(
            time_dists[past_mask],
            same_id_edges[0][past_mask],
            dim=0,
            dim_size=self.graph_obj.num_nodes,
        )[1]
        original_node_ixs = torch.cat(
            (same_id_edges[1][past_mask], torch.as_tensor([-1], device=same_id.device))
        )  # -1 at the end for nodes that were not present
