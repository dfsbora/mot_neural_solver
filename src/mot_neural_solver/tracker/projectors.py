    ...
    def project(self):
        ...
        for viol_constr in all_violated_constr:
            ...
            if round_preds[flow_edges_ix].sum() > 1:
                max_pred_ix = max(
                    flow_edges_ix,
                    key=lambda ix: self.final_graph.edge_preds[ix] * round_preds[ix],
        ...
        assert (
            scatter_add(
                round_preds, self.final_graph.edge_index[1], dim_size=self.num_nodes
            ).max()
            <= 1
        )
        assert (
            scatter_add(
                round_preds, self.final_graph.edge_index[0], dim_size=self.num_nodes
            ).max()
            <= 1
        )
...
class ExactProjector:
    ...
    def project(self):
        ...
        edges_mask = (
            nodes_mask[self.final_graph.edge_index[0]]
            | nodes_mask[self.final_graph.edge_index[1]]
        )
        if edges_mask.sum() > 0:
            ...
            self.final_graph.edge_preds[~edges_mask] = (
                round_preds[~edges_mask].cpu().numpy()
            )
...
class PuLPMinCostFlowSolver:
    ...
    def __init__(self, graph):
        ...
        ).all(), "Cannot project a graph with duplicated edges!"
        ...
        self.edge_vals = {
            edge: edge_val for edge, edge_val in zip(self.edges, self.graph.edge_preds)
        }
    ...
    def _add_constraints(self):
        ...
        for node in self.nodes:
            ...
            inc_flow = plp.lpSum(
                (self.model_vars[tuple(edge)] for edge in in_flow_edges)
            )
            self.m.addConstraint(
                plp.LpConstraint(e=inc_flow, sense=plp.LpConstraintLE, rhs=1)
            )

            out_flow_mask = node_mask[self.graph.edge_index[0]]
            out_flow_edges = self.graph.edge_index.T[out_flow_mask]
            out_flow = plp.lpSum(
                (self.model_vars[tuple(edge)] for edge in out_flow_edges)
            )
            self.m.addConstraint(
                plp.LpConstraint(e=out_flow, sense=plp.LpConstraintLE, rhs=1)
            )
    ...
    def solve(self):
        ...
        self.model_vars = {
            edge: plp.LpVariable(
                lowBound=0, upBound=1, cat=plp.LpContinuous, name=str(edge)
            )
        ...
        obj = plp.lpSum(
            self.model_vars[edge] * (1 - 2 * edge_val)
            for edge, edge_val in self.edge_vals.items()
        )
        ...
        self.graph.edge_preds = np.array(
            [self.model_vars[edge].varValue for edge in self.edges]
        )
