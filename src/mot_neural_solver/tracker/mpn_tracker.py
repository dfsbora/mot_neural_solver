...
class MPNTracker:
    ...
    def _estimate_frames_per_graph(self, seq_name):
        ...
        expected_frames_per_graph = round(
            self.dataset.dataset_params["max_detects"] / avg_detects_per_frame
        )

        return min(
            expected_frames_per_graph, self.dataset.dataset_params["frames_per_graph"]
        )
    ...
    def _load_full_seq_graph_object(
        self, seq_name, start_frame, end_frame, max_frame_dist
    ):
        """
        ...
        if self.use_gt:  # For debugging purposes and obtaining oracle results
            ...
            with torch.no_grad():
                pruned_edge_preds = torch.sigmoid(
                    self.graph_model(subgraph)["classified_edges"][-1].view(-1)
                )
    ...
    def _evaluate_graph_in_batches(self, subseq_graph, frames_per_graph):
        ...
        frame_num_per_node = torch.from_numpy(subseq_graph.graph_df.frame.values).to(
            device
        )
        ...
        for eval_round, (start_frame, end_frame) in enumerate(
            ...
            assert (
                (start_frame <= all_frames) & (all_frames <= end_frame)
            ).sum() == frames_per_graph
            ...
            edges_mask = (
                nodes_mask[subseq_graph.graph_obj.edge_index[0]]
            ...
            subgraph = Graph(
                ...
                edge_index=subseq_graph.graph_obj.edge_index.T[edges_mask].T
            ...
            assert (
                overall_num_preds[torch.where(edges_mask)[0][pred_mask]]
                == overall_num_preds[edges_mask][pred_mask]
            ).all()
    ...
    def _project_graph_model_output(self, subseq_graph):
        ...
        if self.eval_params["rounding_method"] == "greedy":
            ...
            projector = ExactProjector(
                subseq_graph, solver_backend=self.eval_params["solver_backend"]
            )
    ...
    def _assign_ped_ids(self, subseq_graph):
        ...
        graph_shape = (
            subseq_graph.graph_obj.num_nodes,
            subseq_graph.graph_obj.num_nodes,
        )
        csr_graph = csr_matrix(
            (nonzero_edges, (tuple(nonzero_edge_index))), shape=graph_shape
        )

        # Get the connected Components:
        n_components, labels = connected_components(
            csgraph=csr_graph, directed=False, return_labels=True
        )
        assert (
            len(labels) == subseq_graph.graph_df.shape[0]
        ), "Ped Ids Label format is wrong"
        ...
        final_projected_output = final_projected_output[
            VIDEO_COLUMNS + ["conf", "detection_id"]
        ].copy()
    ...
    def _determine_seq_cutpoints(self, seq_name):
        ...
        assert (
            self.eval_params["max_dets_per_graph_seq"]
            > 2 * seq_det_df.groupby("frame")["bb_left"].count().max()
        ), assert_str

        num_dets = seq_det_df.shape[0]
        frame_cutpoints = [
            seq_det_df.frame.iloc[i]
            for i in np.arange(0, num_dets, self.eval_params["max_dets_per_graph_seq"])
        ]
    ...
    def _merge_subseq_dfs(self, subseq_dfs):
        ...
        for subseq_df in subseq_dfs[1:]:
            ...
            left_df = seq_df[["detection_id", "ped_id"]][
                seq_df.frame.isin(intersect_frames)
            ]
            ...
            right_df = subseq_df[["detection_id", "ped_id"]][
                subseq_df.frame.isin(intersect_frames)
            ]
            right_ids_pos = right_df[["ped_id"]].drop_duplicates()
            right_ids_pos["ped_id_pos"] = np.arange(right_ids_pos.shape[0])
            right_df = right_df.merge(right_ids_pos, on="ped_id").set_index(
                "detection_id"
            )
            ...
            common_boxes = (
                left_df[["ped_id_pos"]]
                .join(right_df["ped_id_pos"], lsuffix="_left", rsuffix="_right")
                .dropna(thresh=2)
                .reset_index()
                .groupby(["ped_id_pos_left", "ped_id_pos_right"])["detection_id"]
                .count()
            )
            ...
            cost_mat = np.full(
                (
                    common_boxes["ped_id_pos_left"].max() + 1,
                    common_boxes["ped_id_pos_right"].max() + 1,
                ),
                fill_value=np.nan,
            )
            cost_mat[
                common_boxes["ped_id_pos_left"].values,
                common_boxes["ped_id_pos_right"].values,
            ] = -common_boxes["detection_id"].values
            matched_left_ids_pos, matched_right_ids_pos = solve_dense(cost_mat)
            matched_ids = pd.DataFrame(
                data=np.stack(
                    (
                        left_ids_pos["ped_id"].values[matched_left_ids_pos],
                        right_ids_pos["ped_id"].values[matched_right_ids_pos],
                    )
                ).T,
            ...
            subseq_df = pd.merge(
                ...
                how="outer",
                left_on="ped_id",
                right_on="right_ped_id",
            )
            subseq_df["left_ped_id"].fillna(np.inf, inplace=True)
            subseq_df["ped_id"] = np.minimum(
                subseq_df["left_ped_id"], subseq_df["ped_id"]
            )

            # Update seq_df
            seq_df = pd.concat(
                [seq_df, subseq_df[subseq_df["frame"] > seq_df["frame"].max()]]
            )
    ...
    def track(self, seq_name, output_path=None):
        ...
        for start_frame, end_frame in zip(frame_cutpoints[:-1], frame_cutpoints[1:]):
            ...
            subseq_graph = self._load_full_seq_graph_object(
                seq_name,
                start_frame
                if start_frame == frame_cutpoints[0]
                else start_frame - max_frame_dist,
                end_frame,
                max_frame_dist,
            )

            # Feed graph through MPN in batches
            subseq_graph = self._evaluate_graph_in_batches(
                subseq_graph, frames_per_graph
            )
    ...
    def _add_tracktor_detects(self, seq_df, seq_name):
        def ensure_detects_can_be_used(start_end_per_ped_id):
            ...
            """
            if (
                start_end_per_ped_id.shape[0] == 1
            ):  # If there is a single detection there is nothing to check
                return True

            start_end_per_ped_id_ = start_end_per_ped_id.sort_values(by="min")

            comparisons = start_end_per_ped_id_["min"].values.reshape(
                -1, 1
            ) <= start_end_per_ped_id_["max"].values.reshape(1, -1)
            triu_ixs, tril_ixs = np.triu_indices_from(
                comparisons
            ), np.tril_indices_from(comparisons, k=-1)
        ...
        assert (
            complete_df.shape[0] == big_dets_df.shape[0]
        ), "Merging to add tracktor detects did not work properly"
        ...
        for tracktor_id in unique_tracktor_ids:
            detects_per_tracktor_id = complete_df.loc[tracktor_id][
                ["detection_id", "ped_id", "frame"]
            ]

            if not isinstance(
                detects_per_tracktor_id, pd.Series
            ):  # If there is a single detect, then there's nothing to do
                ...
                start_end_per_ped_id = (
                    detects_per_tracktor_id[detects_per_tracktor_id.ped_id.notnull()]
                    .groupby(["ped_id"])["frame"]
                    .agg(["min", "max"])
                )
                # Good ONe
                ...
                if ensure_detects_can_be_used(start_end_per_ped_id):
                    ...
                    for ped_id, (
                        start_frame,
                        end_frame,
                    ) in start_end_per_ped_id.iterrows():
                        ixs = np.where(
                            detects_per_tracktor_id["frame"].between(
                    ...
                    if self.eval_params["use_tracktor_start_ends"]:
                        ...
                        if len(assigned_ped_ids_ixs) > 0:
                            first_ped_id_ix, last_ped_id_ix = (
                                assigned_ped_ids_ixs.min(),
                                assigned_ped_ids_ixs.max(),
                            )
                    ...
                    assign_ped_ids_ixs = sorted(
                        np.where(detects_per_tracktor_id.ped_id.notnull())[0]
                    )
                    assign_ped_ids = detects_per_tracktor_id.iloc[assign_ped_ids_ixs][
                        "ped_id"
                    ]
                    changes = np.where((assign_ped_ids[:-1] - assign_ped_ids[1:]) != 0)[
                        0
                    ]
                    # build_intervals
                    ...
                    for change in changes:
                        interv_dict[curr_ped_id].append(
                            np.arange(start_ix, assign_ped_ids_ixs[change] + 1)
                        )

                        start_ix = assign_ped_ids_ixs[
                            change + 1
                        ]  # Next ped id appearance
                    ...
                    if self.eval_params["use_tracktor_start_ends"]:
                        if len(assign_ped_ids_ixs) > 0:
                            first_ped_id_ix, last_ped_id_ix = (
                                assign_ped_ids_ixs[0],
                                assign_ped_ids_ixs[-1],
                            )
                            ped_ids[: first_ped_id_ix + 1] = ped_ids[first_ped_id_ix]
                            ped_ids[last_ped_id_ix + 1 :] = ped_ids[last_ped_id_ix]
        ...
        final_out = (
            final_out.groupby(["frame", "frame_path", "ped_id"]).mean().reset_index()
        )
        # print("After groupby ok")
        assert (
            final_out[["frame", "ped_id"]].drop_duplicates().shape[0]
            == final_out.shape[0]
        )
        # print("Assert ok")
