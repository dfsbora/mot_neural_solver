...
class Postprocessor:
    ...
    def drop_short_trajectories(self):
        ...
        short_tracklets_ix = self.traj_df.index[
            self.traj_df.groupby("ped_id")["frame"].transform("count")
            < self.eval_params["min_track_len"]
        ]
    ...
    def interpolate_trajectories(self):
        ...
        for ped_id, (traj_start, traj_end) in traj_start_ends.iterrows():
            if ped_id != -1:
                full_traj_df = pd.DataFrame(
                    data=np.arange(traj_start, traj_end + 1), columns=["frame"]
                )
                partial_traj_df = reixed_traj_df.loc[ped_id].reset_index()

                # Interpolate bb centers, heights and widths
                full_traj_df = pd.merge(
                    full_traj_df,
                    partial_traj_df[
                        ["ped_id", "frame", "mid_x", "mid_y", "bb_height", "bb_width"]
                    ],
                    how="left",
                    on="frame",
                )
        ...
        self.traj_df["bb_right"] = (
            self.traj_df["mid_x"] + 0.5 * self.traj_df["bb_width"]
        )
