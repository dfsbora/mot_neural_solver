...
def get_run_str_and_save_dir(run_id, cross_val_split, add_date):
    run_str = get_run_str(run_id, cross_val_split, add_date=add_date)
    unique_id_assert = (
        f"Run ID string {run_str} already exists, try setting add_date_to_run_str=True"
    )
    ...
    def on_epoch_end(self, trainer, pl_module):
        if trainer.current_epoch + 1 >= self.save_epoch_start and self.save_every_epoch:
            filepath = osp.join(
                ...
                "checkpoints",
                f"epoch_{trainer.current_epoch+1:03}.ckpt",
            )
