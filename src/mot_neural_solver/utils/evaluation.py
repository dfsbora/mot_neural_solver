...
MOT_METRICS_NAMEMAP.update(
    {"norm_" + key: "norm_" + val for key, val in MOT_METRICS_NAMEMAP.items()}
)
MOT_METRICS_FORMATERS.update(
    {"norm_" + key: val for key, val in MOT_METRICS_FORMATERS.items()}
)
...
class MOTMetricsLogger(Callback):
    ...
    def _compute_mot_metrics(self, epoch_num, pl_module, oracle_results=False):
        constr_satisf_rate = pl_module.track_all_seqs(
        ...
        # Compute MOT Metrics
        mot_metrics_summary = compute_mot_metrics(
            gt_path=osp.join(DATA_PATH, "MOT_eval_gt"),
    ...
    def on_train_start(self, trainer, pl_module):
        self.available_data = (
            len(trainer.val_dataloaders) > 0 and len(trainer.val_dataloaders[0]) > 0
        )
    ...
    def on_epoch_end(self, trainer, pl_module):
        ...
        if self.available_data:
            ...
            metrics_path = osp.join(
                self.output_metrics_dir, f"epoch_{trainer.current_epoch + 1:03}.npy"
            )
            ...
            if self.compute_oracle_results:
                for metric in pl_module.hparams["eval_params"]["mot_metrics_to_norm"]:
                    mot_metrics_summary["norm_" + metric] = (
                        mot_metrics_summary[metric] / trainer.oracle_metrics[metric]
                    )
...
class CrossValidationEvaluator:
    ...
    def _get_per_split_paths(self):
        ...
        pattern = (
            "(\d\d-\d\d_\d\d:\d\d_)?" + self.run_id + "_split_[0-9]"
        )  # Optional date + tag_name + split
        dir_candidates = [
            dir_name
            for dir_name in os.listdir(self.path_to_search)
    ...
    def get_metrics_data(self):
        ...
        print(
            f"Retrieving metrics from experiments with Run IDs: {sorted(per_split_metrics_dir)}"
        )
        ...
        for dir in per_split_metrics_dir:
            per_epoch_files = os.listdir(
                osp.join(self.path_to_search, dir, "mot_metrics")
            )
            for file in per_epoch_files:
                split_iter_metrics = load_pickle(
                    osp.join(self.path_to_search, dir, "mot_metrics", file)
                )
                split_iter_metrics = pd.DataFrame(split_iter_metrics)
                split_iter_metrics = (
                    split_iter_metrics.drop("OVERALL")
                    .reset_index()
                    .rename(columns={"index": "scene"})
                )
    ...
    def _compute_per_epoch_MOTA_and_prec(self, metrics_df):
        ...
        scenes_per_iter = (
            ...
            .groupby("epoch_num")["scene"]
            .agg(lambda x: tuple(x.unique()))
        )
        ...
        per_epoch_overall_vals["has_all_scenes"] = per_epoch_overall_vals[
            "scene"
        ].apply(lambda x: set(all_scenes).issubset(x))

        if (
            "constr_sr" in per_epoch_overall_vals
            and per_epoch_overall_vals["constr_sr"].isnull().sum() == 0
        ):
            per_epoch_overall_vals["constr_sr"] = metrics_df.groupby(["epoch_num"])[
                "constr_sr"
            ].mean()

        # Compute MOTA and MOTA Log
        per_epoch_overall_vals["mota"] = 100 * (
            1
            - (
                per_epoch_overall_vals["num_misses"]
                + per_epoch_overall_vals["num_false_positives"]
                + per_epoch_overall_vals["num_switches"]
            )
            / per_epoch_overall_vals["num_objects"]
        )
        per_epoch_overall_vals["MOTA Log"] = 100 * (
            1
            - (
                per_epoch_overall_vals["num_misses"]
                + per_epoch_overall_vals["num_false_positives"]
                + np.log10(per_epoch_overall_vals["num_switches"])
            )
            / per_epoch_overall_vals["num_objects"]
        )
        ...
        per_epoch_overall_vals["idr"] = (
            100
            * per_epoch_overall_vals["idtp"]
            / (per_epoch_overall_vals["idtp"] + per_epoch_overall_vals["idfn"])
        )
        per_epoch_overall_vals["idf1"] = (
            100
            * 2
            * per_epoch_overall_vals["idtp"]
            / (
                per_epoch_overall_vals["num_predictions"]
                + per_epoch_overall_vals["num_objects"]
            )
        )
    ...
    def _choose_best_epoch_results(self, per_epoch_metrics, best_method_metric):
        ...
        best_iter = max(
            valid_indices, key=lambda x: per_epoch_metrics.loc[x][best_method_metric]
        )
    ...
    def evaluate(self, cols_to_norm, best_method_metric):
        ...
        for col in cols_to_norm:
            per_epoch_metrics["norm_" + col] = (
                100 * per_epoch_metrics[col] / oracle_metrics[col].iloc[0]
            )

        best_iter, best_row, best_metric_val = self._choose_best_epoch_results(
            per_epoch_metrics, best_method_metric
        )
...
def fast_compute_class_metric(
    test_preds, test_sols, class_metrics=("accuracy", "recall", "precision")
):
    """
...
def compute_constr_satisfaction_rate(
    graph_obj, edges_out, undirected_edges=True, return_flow_vals=False
):
    """
    ...
    if undirected_edges:
        ...
        sorted = sorted.t()
        div_factor = 2.0  # Each edge is predicted twice, hence, we divide by 2
    else:
        sorted = edge_ixs  # Edges (i.e. node pairs) are already sorted
        div_factor = 1.0  # Each edge is predicted once, hence, hence we divide by 1.

    # Compute incoming and outgoing flows for each node
    flow_out = (
        scatter_add(edges_out, sorted[0], dim_size=graph_obj.num_nodes) / div_factor
    )
    flow_in = (
        scatter_add(edges_out, sorted[1], dim_size=graph_obj.num_nodes) / div_factor
    )
...
def compute_perform_metrics(graph_out, graph_obj):
    ...
    class_metrics["constr_sr"] = compute_constr_satisfaction_rate(
        graph_obj, edges_out=edges_out
    )
